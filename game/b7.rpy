...
init python:
    show_error = False
    row_number = 0
    ev_al_task1_label = "Представьте себя в роли эксперта, оценивающего указанные альтернативы. Альтернативы сравниваются попарно по указанной шкале. Результат заполнения заносится в таблицу. Правило сравнения:"
    ev_al_task1_info = "1 - Aстроки и Aстолбца примерно равноценны;\n3 - Aстроки немного предпочтительнее Aстолбца или 0.33, если Астолбца немного предпочтительнее Астроки;\n5 - Aстроки предпочтительнее Aстолбца или 0.2, если Астолбца предпочтительнее Астроки;\n7 - Aстроки значительно предпочтительнее Aстолбца или 0.14, если Астолбца предпочтительнее Астроки;\n9 - Aстроки явно предпочтительнее Aстолбца или 0.11, если Астолбца явно предпочтительнее Астроки."
    ev_al_task1_info += "\nМожно ставить промежуточные баллы."
    ev_al_task1_alternatives = method1_task1_alternatives[:]
    ev_al_task1_alternatives_info = "Альтерантивы:"
    ev_al_task2_label = "Теперь необходимо провести обработку этих данных. Для каждой строки матрицы попарных сравнений определяется цена альтернативы Ci - среднее геометрическое строки матрицы. Определяется сумма цен альтернатив: C = ΣCi. Определяется вес каждой альтернативы: Vi = Ci/C и ее ранг Ri, i=1,...,N."
